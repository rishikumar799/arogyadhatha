...
const NotificationItem = ({ notification, language }: { notification: (typeof notifications)[0], language: 'en' | 'te' }) => {
    ...
    useEffect(() => {
        setTimeAgo(format(new Date(notification.timestamp), 'dd-MMM-yyyy, h:mm a'));
...
export default function NotificationsPage() {
    ...
    }, [filteredNotifications, t.today, t.yesterday, isClient]);
                ...
                {Object.keys(groupedNotifications).length > 0 ? (
                    ...
                    <Card className="text-center p-12 border">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            {isClient ? t.noNotificationsInCategory.replace('{filter}', filter) : 'Loading...'}
                        </p>
                    </Card>
                )}
            </div>
