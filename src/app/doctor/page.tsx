  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-6">
        <h2 className="text-xl font-semibold mb-2">Welcome, Doctor</h2>
        {user ? (
          <p>Hello, <span className="font-medium">{user.email}</span>. You have successfully accessed your protected dashboard.</p>
        ) : (
          <p>Loading user data...</p>
        )}
        <p className="mt-4 text-sm text-gray-600 dark:text-gray-400">This page is only visible to authenticated users with the 'doctor' role.</p>
      </div>
    </div>
  );
}
